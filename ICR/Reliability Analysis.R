...
setwd("/Users/lukefischer/Desktop/Content Analysis/Content Analysis") # Please update the working directory accordingly

# Reliability Step 1 ----------------------------------------------------------

set.seed(123)

headers2 <- read.csv("reliability_step1.csv", nrows=1, header= F) # Create column names for data from reliability step 1

dataset2 <-read.csv("reliability_step1.csv", skip=3, header = F) # Load data from reliability step 1
...
summary(alpha_presence) # Reliability for "Responsiblity Attribution Presence"

improvement_presence <- influence(alpha_presence, units = seq(1:38))

bind_cols(match_set, improvement_presence) |> # Check which posts diminished reliability
  filter(dfbeta.units < 0)|> 
  mutate(dfbeta.units = round(dfbeta.units, digits = 3)) |> 
  filter(dfbeta.units < 0) 

improvement_presence2 <- influence(alpha_presence, coders = c(1:4)) # Check which coder(s) diminished reliability
...
headers <- read.csv("reliability_step2.csv", nrows=1, header= F) # Create column names for data from reliability step 2

dataset <-read.csv("reliability_step2.csv", skip=3, header = F) # load data from reliability step 2
...
### RESPONSIBILITY ATTRIBUTION TYPE (Excluded from final codebook)
pivot_type <- dataset |> 
...
## SENDER-ACCOUNT-MATCH (Sender Relevance)
...
## RESPONSIBILITY ATTRIBUTION EVALUATION
...
## RESPONSIBILITY ATTRIBUTION TARGET

### Presence individual (In final paper, part of overall target category)
...
#### Specific systems (Excluded from the final codebook)
...
### Specific network (Excluded from final codebook)
...
##RESPONSIBILITY ATTRIBUTION TOPIC
...
## RESPONSIBILITY ATTRIBUTION TIME (all excluded from final codebook)
